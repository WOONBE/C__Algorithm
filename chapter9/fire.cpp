...
int main(void){
    ...
    while(!Q2.empty()){
        ...
        for(int dir = 0; dir < 4; dir++){
            ...
            if(dist2[nx][ny] >= 0 || board[nx][ny] == '#') continue; //들렀던 곳이거나 벽이면 continue
            if(dist1[nx][ny] != -1 && dist1[nx][ny] <= dist2[cur.X][cur.Y]+1) continue; // 불의 전파 시간을 조건에 추가 (못지나감)
